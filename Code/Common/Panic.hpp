...
namespace x {
    namespace impl {
        [[noreturn]] inline void Panic(const char* file, int line, const char* func, const char* msg) noexcept {
            ...
            std::ignore = fprintf(stderr, msgFormatted);
            #ifndef NDEBUG
        ...
        [[noreturn]] void
        Panic(const char* file, int line, const char* func, const char* fmt, Args... args) noexcept {
            char msg[1024];
...
#define PANIC_IF_FAILED(hr, fmt, ...) \
        if (FAILED(hr)) {\
        x::impl::Panic(__FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); \
        }